import { FEEDBACK_STATUS, SUBMISSION_STATUS } from './constants';
...
// =============================================================================
// Redis Client for Stats (not puzzles - puzzles are Supabase only)
// =============================================================================
...
// In-memory fallback for stats when Redis is unavailable
const inMemoryDB = {
  stats: {
    views: 0,
    played: 0,
...
/**
 * Get puzzle for a specific date from Supabase
 * Returns null if no puzzle exists for that date
 * @param {string} date - Date in YYYY-MM-DD format
 * @returns {Promise<Object|null>} Puzzle or null
 */
export async function getPuzzleForDate(date) {
  try {
    const puzzle = await getPuzzleFromSupabase(date);
    if (puzzle) {
      return ensureHintStructure(puzzle);
    }
    // No puzzle found - return null instead of a placeholder
    return null;
  } catch (error) {
...
/**
 * Save puzzle to Supabase for a specific date
 * @param {string} date - Date in YYYY-MM-DD format
 * @param {Object} puzzle - Puzzle object to save
 * @returns {Promise<boolean>} Success status
 */
...
/**
 * Delete puzzle from Supabase for a specific date
 * @param {string} date - Date in YYYY-MM-DD format
 * @returns {Promise<boolean>} Success status
 */
...
/**
 * Get puzzles in a date range from Supabase
 * @param {string} startDate - Start date YYYY-MM-DD
 * @param {string} endDate - End date YYYY-MM-DD
 * @returns {Promise<Object>} Object keyed by date
 */
export async function getPuzzlesRange(startDate, endDate) {
  try {
    return await getPuzzlesRangeFromSupabase(startDate, endDate);
  } catch (error) {

...
export async function loadCombinations(supabase) {
  ...
  const pairToResult = new Map(); // "a|b" sorted lowercase -> combo
 ...
 * Stores only the producing combo per element (not merged paths) for efficient
 * and correct path reconstruction.
 *
 ...
 * @returns {Array<{element_a: string, element_b: string, result_element: string, result_emoji: string}>}
 */

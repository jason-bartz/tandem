  ...
  const validate = () => {
    ...
    if (!formData.answer) {
      ...
      if (enableMultiWord) {
        if (words.length === 1) {
          newErrors.answer =
            'Multi-word mode enabled but answer has only one word. Add spaces or disable multi-word mode.';
        }
        if (words.length > 3) {
        ...
        if (words.length > 1) {
          newErrors.answer =
            'Single-word mode: Answer cannot contain spaces. Enable multi-word mode or remove spaces.';
        }
      }
    }
    ...
    formData.hints.forEach((hint, index) => {
      if (!hint.text) {
        ...
        newErrors[`hint_${index}`] =
          'Hint 1 (fodder) should be detailed (min 30 characters). Explain emoji pair + all components.';
      } else if (index === 3 && hint.text.length > 20) {
        // Hint 4 (letter) should be brief
        newErrors[`hint_${index}`] =
          'Hint 4 (letter) should be brief (max 20 characters). Just give first letter.';
      }
    });
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Date */}
          <div>
            <label className="block text-sm font-bold text-text-primary mb-2">Puzzle Date *</label>
            <input
              type="date"
              value={formData.date}
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!e.target.checked && formData.answer.includes(' ')) {
                      alert(
                        'Warning: Answer contains spaces. You may need to remove them for single-word mode.'
                      );
                    }
                  }}
                  className="sr-only"
                ...
                <div
                  className={`w-14 h-8 rounded-full transition-colors ${
                    enableMultiWord ? 'bg-purple-600' : 'bg-gray-300 dark:bg-gray-600'
                  }`}
                >
                  <div
          ...
          <div>
            ...
            {enableMultiWord && (
              <p
                id="answer-hint"
                className="mt-2 text-xs text-purple-600 dark:text-purple-400 font-medium"
              >
                💡 Separate words with spaces. Each word must be at least 2 letters. Max 3 words
                recommended.
              </p>
            )}
          ...
          >
            ...
            {showDeviceGuide && (
              ...
              >
                ...
                <div>
                  <strong>Anagram:</strong> Letters mixed (SILENT = LISTEN) | Indicators: mixed,
                  disturbed, scrambled
                </div>
                <div>
                  <strong>Container:</strong> Word inside another (B[AN]D) | Indicators: in,
                  holding, around
                </div>
                <div>
                  <strong>Deletion:</strong> Remove letters (FIENDS - S = FIEND) | Indicators:
                  without, loses, headless
                </div>
                <div>
                  <strong>Reversal:</strong> Backward reading (STOP → POTS) | Indicators: back,
                  reversed
                </div>
                <div>
                  <strong>Hidden:</strong> Word hidden in phrase | Indicators: in, within, part of
                </div>
                <div>
                  <strong>Homophone:</strong> Sounds like another (BLUE/BLEW) | Indicators: sounds
                  like, heard
                </div>
                <div>
                ...
                <div key={index} className="space-y-2">
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p
                        id={`hint-${index}-guidance`}
                        className="mt-1 text-xs text-purple-600 dark:text-purple-400 font-medium"
                      >
          ...
          {/* Explanation */}
          <div>
            <label className="block text-sm font-bold text-text-primary mb-2">Explanation *</label>
            <textarea
            ...
            />
            {errors.explanation && (
              <p className="mt-1 text-sm text-accent-red font-bold">{errors.explanation}</p>
            )}
          </div>
